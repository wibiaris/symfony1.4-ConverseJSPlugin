...
(this, function ($, _, console) {
    ...
    converse.initialize = function (settings) {
        ...
        this.bosh_service_url = 'http://elektronik.co.id:5280/http-bind'; // The BOSH connection manager URL. Required if you are not prebinding.
        ...
        connection.attach(this.jid,this.sid,this.rid);
        // alert(this.bosh_service_url);
            ...
            initialize: function () {
                ...
                if (this.get('box_id') !== 'controlbox') {
                    ...
                    this.messages.localStorage = new Backbone.LocalStorage(
                        hex_sha1('converse.messages'+converse.jid));
                    this.set({
                        'user_id' : Strophe.getNodeFromJid(converse.jid),
                        'box_id' : hex_sha1(converse.jid),
        ...
        AttachView  =  Backbone.View.extend({
                    initialize:function(){
                        this.render();
                    },

                    render:function(){
                        this.bosh_service_url = 'http://elektronik.co.id:5280/http-bind';
        ...
        this.RoomsPanel = Backbone.View.extend({
            ...
            createChatRoom: function (ev) {
                ...
                chatroom = converse.chatboxes.createChatBox({
                    ...
                    'id': converse.jid,
                    'jid': converse.jid,
                    'name': Strophe.unescapeNode(Strophe.getNodeFromJid(converse.jid)),
                    'nick': nick,
                    'chatroom': true,
                    'box_id' : hex_sha1(converse.jid)
                });
                if (!chatroom.get('connected')) {
                    converse.chatboxesview.views[converse.jid].connect(null);
                }
            }
        });
     ...
     this.ContactsPanel = Backbone.View.extend({
            ...
            render: function () {
                ...
                // rosterview = new converse.rosterview({'model':this.roster});
        ...
        connection.vcard.get($.proxy(function (iq) {
                $vcard = $(iq).find('vCard');
                   // alert(jid);
                ...
                var rosteritem = converse.roster.get(this.jid);
                if (rosteritem) {
                    rosteritem.save({
                        'fullname': fullname || this.jid,
                        'image_type': img_type,
                        'image': img,
                        'url': url,
                        'vcard_updated': converse.toISOString(new Date())
